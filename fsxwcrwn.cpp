...
double Crown::CrownSpread(double avgros, double CrownFireSpreadRate) //R10)		 // R10 is from Rothermel 1991
{
	double CrownROS = 0.0;
	...
	if (CrownBulkDensity > 0.0)
	{
		...
		/*
			...
			if(CFB<0.9)
				CrownROS=0.0;
			*/
		//JAS! ver -- CrownROS = avgros + CrownFractionBurned * (R10 - avgros);	// Van Wagner's 2-equation crown fire model
   CrownROS=avgros+CrownFractionBurned*(CrownFireSpreadRate-avgros);  // Van Wagner's 2-equation crown fire model -- FromG5 ver
   if( GetCrownFireCalculation()==0)  // do only for original FARSITE calculations -- In FromG5 ver only
...
//Defined in FromG5 ver only below
