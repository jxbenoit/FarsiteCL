/*fsxwmech.cpp
  Mechanix model functions for FARSITE.
...
{ //MechCalls::LoadGlobalFEData
  ...
  m_livew = gmw.livew;
  m_liveh = gmw.liveh;
//printf("AAA fsxwmech:MechCalls::LoadGlobalFEData 1 m_ones=%f m_tens=%f m_hundreds=%f m_liveh=%f m_livew=%f\n",m_ones,m_tens,m_hundreds,m_liveh,m_livew);
  ...
  if( m_winddir == -1.0 ) m_winddir = ld.aspectf;
  else if( m_winddir < -1.0 ) m_winddir = ( PI - ld.aspectf );
...
{ //MechCalls::GetPoints
  ...
  if( CurrentPoint == 0 ) {
    ...
    xpt = GetPerimeter2Value( CurrentPoint, XCOORD );
    ypt = GetPerimeter2Value( CurrentPoint, YCOORD );
  ...
  if( GetInout(CurrentFire) == 1 ) return;
...
{ //MechCalls::NormalizeDirectionWithLocalSlope
	double Theta;
	double Angle;
	double hx, hy, ds;
...
{ //MechCalls::AccelSurf1
//printf("AAA fsxwmech:MechCalls::AccelSurf1 1 xt=%f\n",xt);

  // does first acceleration of fire, computes Ros at T+1 given time step

  //if negative ros (initial fraction of equilibrium ROS at input only)

  if( RosT < 0.0 ) RosT *= (-vecros);
  if( xt == 0.0 && yt == 0.0 ) vecros = RosT;
  else SpreadCorrect();  // vecros=equilibrium spread rate in vectored direction
...
//printf("AAA fsxwmech:MechCalls::AccelSurf1 1a xt=%f SpreadRatio=%f\n",xt,SpreadRatio);
...
//printf("AAA fsxwmech:MechCalls::AccelSurf1 2 xt=%f\n",xt);
...
{ //MechCalls::AccelCrown1
//printf("AAA fsxwmech:MechCalls::AccelCrown1 1 xt=%f\n",xt);
...
//printf("AAA fsxwmech:MechCalls::AccelCrown1 2 xt=%f\n",xt);
...
{ //MechCalls::SlopeCorrect
//printf("AAA fsxwmech:MechCalls::SlopeCorrect 1 midx=%f xpt=%f midy=%f ypt=%f xt=%f\n",midx,xpt,midy,ypt,xt);
  ...
  if( ActualSurfaceSpread > 0.0) { // Expansion Rate
		...
		else cosslope = 1.0;		// slope is 0 then cosine is 1.0
//printf("AAA fsxwmech:MechCalls::SlopeCorrect 1a midx=%f xpt=%f midy=%f ypt=%f\n",midx,xpt,midy,ypt);

		xpt = midx - (timerem * xt * RosReduct * MetricResolutionConvert()); // computes new horizontal xpt
//printf("AAA fsxwmech:MechCalls::SlopeCorrect 1b timerem=%f xt=%f RosReduct=%f\n",timerem,xt,RosReduct);

//printf("AAA fsxwmech:MechCalls::SlopeCorrect 1c MetricResolutionConvert=%f\n",MetricResolutionConvert());

//printf("AAA fsxwmech:MechCalls::SlopeCorrect 1d xt=%f\n",xt);

		ypt = midy - (timerem * yt * RosReduct * MetricResolutionConvert()); // and ypt coordinates
	}
//printf("AAA fsxwmech:MechCalls::SlopeCorrect 2 midx=%f xpt=%f midy=%f ypt=%f\n",midx,xpt,midy,ypt);
...
{ //MechCalls::VecSurf
  if( ld.slope > 0 ) {
    ...
    if( vecspeed > phiew ) vecspeed = phiew; //vecspeed is still dimensionless
    vecros = ( fros * (1.0 + vecspeed) );  //wind/slope vectored spread rate
  }
  else {
    vecdir = m_winddir;
    ivecspeed = m_windspd;
    if( phiw > phiew ) phiw = phiew;
    vecspeed = phiw;
...
//printf("AAA fsxwmech:Mechanix::distchek 1\n");
	...
	else NumPtTotal = -1;  // for debugging
//printf("AAA fsxwmech:Mechanix::distchek 2\n");
...
{ //Mechanix::grow
  ...
  if( xdiff == 0 && ydiff == 0 ) {
    ...
    f2 = pow2( flank );
    h2 = pow2( head );
    part1 = f2 * cos( ivecdir )
               * ( xdiff * sin(ivecdir) + ydiff * cos(ivecdir) );
    part2 = h2 * sin( ivecdir )
               * ( xdiff * cos(ivecdir) - ydiff * sin(ivecdir) );
    part3 = h2 * pow2( (xdiff * cos(ivecdir) - ydiff * sin(ivecdir)) );
    part4 = f2 * pow2( (xdiff * sin(ivecdir) + ydiff * cos(ivecdir)) );
    part5 = f2 * sin( ivecdir )
               * ( xdiff * sin(ivecdir) + ydiff * cos(ivecdir) );
    part6 = h2 * cos( ivecdir )
               * ( xdiff * cos(ivecdir) - ydiff * sin(ivecdir) );
...
{ //Mechanix::limgrow
  ...
  //Computes horizontal distance traveled.
  dist = sqrt( pow2(distx) + pow2(disty) );

  if( dist > MINDIST ) {
    fdist = MINDIST / dist;
    xpt = midx - distx * fdist;  //New xpt and new ypt computed from
    ypt = midy - disty * fdist;  //mindist distance along spread direction
    //Remainder of time step, distance traveled/ros ONLY IF CONSTANT ROS.
    timerem = step - ( MINDIST / (dist / step) );
    //Horizontal spread now equals the distance check.
    HorizSpread = GetDynamicDistRes();
  }
  else timerem = 0.0;  //No time in timestep remaining

  //Keep old point to prevent coincident points on edge of landscape.
...
{ //Mechanix::scorrect
  ...
  if( (aspectf <= PI / 2.0) || (aspectf > PI && aspectf <= 3.0 * PI / 2.0) ) {
    if( cos(Angle) != 0.0 || Angle == PI ) {
      if( tan(Angle) < 0 ) {
        if( tan(Angle) < tan(aspectf - PI / 2.0) ) My = 1.0;
        else Mx = 1.0;
      }
    }
  }
  ...
  xt = hxt * XSign;
  yt = hyt * YSign;
//printf("AAA fsxwmech:MechCalls::scorrect 1 xt=%f\n",xt);
...
{ //Mechanix::spreadrate
  ...
  double seff[4][2]
       ...
       = { {0.01,0.01}, {0.01,0.01}, {0.01,0}, {0.01,0.0} }; //mineral content
  ...
  if( ! GetNewFuel(fuel, &newfuel) ) { //will get all models, 13, 40, & custom
    rateo = 0.0;
    return rateo;
  }
  ...
  if( newfuel.lh ) nlive++;
  if( newfuel.lw ) nlive++;
  if( nlive == 0 ) newfuel.dynamic = 0;
  if( nlive > 0 ) nlive = 2;    // boost to max number
  if( ndead > 0 ) ndead = 4;
  ...
  */
  ...
  for( i = 0; i < 2; i++ ) {
    if( nclas[i] != 0 ) {
      ...
      if( xmf[i] >= xmext[i] ) etam[i] = 0;
      etas[i] = 0.174 / (pow(se[i], 0.19));
      if( etas[i] > 1.0 ) etas[i] = 1.0;
...
//AAA FromG5 ver only below
void Mechanix::GetEquationTerms( double *pphiw, double *pphis, double *bb,
                                 double *ppart1 )
{
  *pphiw=phiw;
  *pphis=phis;
...
{ //Mechanix::ellipse
  ...
  //Calculate Alexanders head/backing ratio assuming fire origin at focus
  //equation [16].
  //Subt 0.397 to make circle at 0 windspd (my own addition).
  double part = sqrt( pow2(lb_ratio) - 1 );
...
void Mechanix::TransformWindDir(long slope, double aspectf)
{
  ...
  double Theta, slopef;       //AAA In FromG5 ver only

  double X, Y, Z, S;//, P;    //AAA In FromG5 ver only
...
//printf("AAA fsxwmech:Mechanix::vectordir 1 aspectf=%f\n",aspectf);
...
//printf("AAA fsxwmech:Mechanix::vectordir 2 angleabs=%f PI=%f angle=%lf phiw=%f phis=%f LocalWindDir=%f\n",angleabs,PI,angle,phiw,phis,LocalWindDir);



  if( angleabs < PI ) {
//printf("AAA fsxwmech:Mechanix::vectordir 2a\n");

    //AAA    if( angle > 0.0 ) {

    if( angle > 0.00001 ) {

      if( phiw >= phis ) vecdir = LocalWindDir - vangle;//m_winddir-vangle;
      else vecdir = aspectf + vangle;
    }
    else {
      if( phiw >= phis ) vecdir = LocalWindDir + vangle;//m_winddir+vangle;
      else vecdir = aspectf - vangle;
  ...
  if( vecdir < 0.0 ) vecdir = 2.0 * PI + vecdir;
