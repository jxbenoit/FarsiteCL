...
{ //FECalls::GetLandscapeData
  CallLevel++;

  if( Verbose >= CallLevel )
    printf( "%*sfsxwenvt2:FECalls::GetLandscapeData:1\n", CallLevel, "" );

  if( xpt < GetLoEast() || xpt > GetHiEast() ||
    ypt < GetLoNorth() || ypt > GetHiNorth() ) {

    if( Verbose >= CallLevel )
      printf( "%*sfsxwenvt2:FECalls::GetLandscapeData:1a\n", CallLevel, "" );
    CallLevel--;
    return 0;
  }

  //Copy coordinates to FireEnvironment::Variables for use with gridded data.
  XLocation = xpt;
  YLocation = ypt;
  ...
  if( Verbose >= CallLevel )
    printf( "%*sfsxwenvt2:FECalls::GetLandscapeData:2\n", CallLevel, "" );
  CallLevel--;
...
{
  if( HaveGroundFuels() ) ld.woody = woody;
  else ld.woody = 0;
}
...
void FECalls::GetFireEnvironment( double SIMTIME, int type, long ReadNewData,
                                  long PointNum )
{ //FECalls::GetFireEnvironment
  CallLevel++;
  long oldfuel = ld.fuel;

  if( Verbose > CallLevel )
    printf( "%*sfsxwenvt2:FECalls:GetFireEnvironment:1\n", CallLevel, "" );
	if( ld.fuel < 0 ) {
		ld.fuel = 51;
		StationNumber = 0;
	}
  if( Verbose > CallLevel )
    printf( "%*sfsxwenvt2:FECalls:GetFireEnvironment:2\n", CallLevel, "" );
  ...
  else NoMoistureData(type);
  if( Verbose > CallLevel )
    printf( "%*sfsxwenvt2:FECalls:GetFireEnvironment:3\n", CallLevel, "" );
