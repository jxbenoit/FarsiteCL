/*burnupw.cpp
  ...
  See Albini and Reinhardt for technical details.
*/
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
...
bool BurnUp::SetFireDat( long NumIter, double Fi, double Ti, double U,
                         double D, double Tamb, double R0, double Dr,
                         double Dt, double Wdf, double Dfm )
{ //BurnUp::SetFireDat
  ntimes = NumIter;
...
{ //BurnUp::Burnup
  ...
  if( ntimes == 0 || number == 0 ) return false;
  ...
  if( ! nohist ) Stash( HistFile, SnapFile, tis, now );
  fi = FireIntensity();
...
{ //BurnUp::Nint
  ...
  Input = (long) input;
  if( input - (double) Input >= 0.5 ) Input += 1;
...
{ //BurnUp::Arrays
  ...
  //c  Orders the fuel description arrays according to the paradigm described
  //c  in subroutine SORTER and computes the interaction matrix xmat from the
  //c  array elam and the list alone returned from subroutine OVLAPS.
  //c  Parameters in arrays are defined in terms of initial values as:
  ...
  for( j = 0; j < number; j++ ) wdry[j] = work[j];
  for( j = 0; j < number; j++ ) {
    k = key[j];
    work[j] = ash[k];
  }
  for( j = 0; j < number; j++ ) ash[j] = work[j];
  for( j = 0; j < number; j++ ) {
    k = key[j];
    work[j] = htval[k];
  }
  for( j = 0; j < number; j++ ) htval[j] = work[j];
  for( j = 0; j < number; j++ ) {
    k = key[j];
    work[j] = cheat[k];
  }
  for( j = 0; j < number; j++ ) cheat[j] = work[j];
  for( j = 0; j < number; j++ ) {
    k = key[j];
    work[j] = condry[k];
  }
  for( j = 0; j < number; j++ ) condry[j] = work[j];
  for( j = 0; j < number; j++ ) {
    k = key[j];
    work[j] = tpig[k];
  }
  for( j = 0; j < number; j++ ) tpig[j] = work[j];
  for( j = 0; j < number; j++ ) {
    k = key[j];
    work[j] = tchar[k];
  }
  for( j = 0; j < number; j++ ) tchar[j] = work[j];

  OverLaps();  //call OVLAPS( wdry, sigma, dendry, ak, number, maxno, maxkl,
               //             xmat, elam, alone )
...
{ //BurnUp::TempF
  ...
  //c  q in kW / square meter, the ambient temperature tamb in Kelvins, and
  //c  the dimensionless mixing parameter r.
...
{ //BurnUp::Step
  ...
  *ncalls += 1;
  ...
  for( k = 1; k <= number; k++ ) { //do k = 1 , number
    ...
    for( l = 0; l <= k; l++ ) { //do l = 0 , k
      ...
      if( tnow >= tlit ) {
        ...
        tst = max( tlit, tifi );
        //nint((tnext-tst)/dt));
        nspan = max( (long) 1, Nint((tnext - tst) / dt) );
        if( nspan <= MXSTEP ) qdot[kl][nspan - 1] = qqq;
        ...
        tavg = tav1;
        if( tav2 < tavg ) tavg = tav2;
        if( tav3 < tavg ) tavg = tav3;
        ...
        do {
          index -= 1;
          if( index == 0 ) deltim = tnext - tspan - tlit;
          if( (tspan + deltim) >= tavg ) deltim = tavg - tspan;
      ...
      if( tnow >= dryt && tnow < tlit ) {
        ...
        if( tfe > (tpig[k - 1] + 10.0) )
          dtlite = TIgnite( tpdry, tpig[k - 1], tfe, condry[k - 1],
                            cheat[k - 1], fmois[k - 1], dendry[k - 1], heff );
        ...
        if( tnext > tign[kl] ) {
          ...
          wnext = wo[kl] * pow2( dnext / dia );
          if( dnext == 0.0 ) tout[kl] = tnow + dia / ddot[kl];
          else if( (dnext > 0.0) && dnext < dia ) {
            rate = dia / ( dia - dnext );
            tout[kl] = tnow + rate * dt;
          }
      ...
      if( tnow < dryt ) {
        ...
        tf = TempF( gi, r );
        if( tf <= (tpdry + 10.0) ) continue;  // goto 10
        dia = diam[kl];
        ts = 0.5 * ( tamb + tpdry );
        HeatExchange( dia, tf, ts, &hf, &hb, c, &e );
  ...
  for( k = 1; k <= number; k++ ) {  // do k = 1 , number
    ...
    for( l = 0; l <= k; l++ ) {  //do l = 0 , k
      kl = loc( k, l );
      if( tnext >= tign[kl] ) flag = true;
      else flag = false;
      if( flag && tnext <= tout[kl] ) flit[k - 1] += xmat[kl];
      if( tnext > tout[kl] ) fout[k - 1] += xmat[kl];
    }
  }
} //BurnUp::Step
...
{ //Burnup::Start
  ...
  for( k = 1; k <= number; k++ ) {  //do k = 1 , number
    ...
    for( l = 0; l <= k; l++ ) {  //do l = 0 , k
      ...
      qcum[kl] = hb * tcum[kl];
      if( tf <= (tigk + 10.0) ) continue;
      dtign = TIgnite( tpdry, tpig[k - 1], tf, condry[k - 1],
                       cheat[k - 1], fmois[k - 1], dendry[k - 1], hb );
      trt = dryt + dtign;
      tign[kl] = 0.5 * trt;
      if( dt > trt ) flit[k - 1] += xmat[kl];
    }
  }
  nlit = 0;
  trt = rindef;
  ...
  for( k = 1; k <= number; k++ ) { //do k = 1 , number
    for( l = 0; l <= k; l++ ) {  //do l = 0 , k
      kl = loc( k, l );
      if( tdry[kl] < rindef ) tdry[kl] -= trt;
      if( tign[kl] < rindef ) tign[kl] -= trt;
    }
  }
...
{ //BurnUp::Sorter
  ...
  for( j = 2; j <= number; j++ ) {  //do j = 2 , number
    ...
    for( i = (j - 2); i >= 0; i-- ) {  //do i = ( j - 1 ) , 1 , -1
      ...
      if( diam ) break;  //goto 10
      if( usi == s ) tied = true;
      else tied = false;
      if( tied ) {  //goto 05
        if( fmois[i] < fm ) mois = true;
        else mois = false;
        if( mois ) break;  //goto 10
        if( fmois[i] == fm ) tied = true;
        else tied = false;
        if( tied ) {
          if( dendry[i] <= de ) dens = true;
          else dens = false;
          if( dens ) break;  //goto 10
        }
      }
      sigma[i + 1] = sigma[i];
      fmois[i + 1] = fmois[i];
...
{ //BurnUp::FireIntensity
  ...
  double wnoduff, wnoduffsum, noduffsum, test, noduffterm;
  double fracf;
  ...
  for( k = 1; k <= number; k++ ) {
    ...
    for( l = 0; l <= k; l++ ) {
      kl = loc( k, l );
      wdotk += wodot[kl];
      //if( l > 0 )  wnoduff += wodot[kl];
      //else Smoldering[k-1] += wodot[kl];
    }
    term = ( 1.0 - ash[k - 1] ) * htval[k - 1] * wdotk * 1.e-03;
    ark = area[k - 1];
    if( ark > small ) fint[k - 1] = term / ark - term;
    else fint[k - 1] = 0.0;
    k0 = loc( k, 0 );
...
{ //BurnUp::Summary
  ...
  if( (outfile = fopen(OutFile, "w")) == NULL ) return false;
...
//============================================================================
long BurnUp::GetSamplePoints( long Flaming, long NumberOfPts )
{ //BurnUp::GetSamplePoints
  long   i, j;
  ...
  do {
    ...
    for( i = 1; i < NumInSample; i++ ) {
      ...
      else ytest = bs[j].ff;  //bpx.fi;
      if( fabs(x[i - 1] - x[i]) > 1e-6 ) {
        ymid = y[i - 1] - ( y[i - 1] - y[i] ) * ( x[i - 1] - xtest ) /
               ( x[i - 1] - x[i] );
...
{ //BurnUp::GetBurnStruct
  if( Num > now - 1 ) return false;
...
long BurnUp::RunPolynomialRegression( long Flaming )
{
  ...
  if( AllocRegressionData(ndata) == false ) return 0;
