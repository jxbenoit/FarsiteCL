/*============================================================================
  ...
  Main class for the Farsite model.
...
//============================================================================
Farsite::Farsite( const char* farsitedirectory )
{ //Farsite::Farsite
  NumSimThreads = 0;
...
//============================================================================
bool Farsite::SetInputsFromFile( char* settingsfilename )
{ //Farsite::SetInputsFromFile
  CallLevel++;
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:SetInputsFromFile:1\n",
            CallLevel, "" );
  ...
  while( ! feof(IN) ) {
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:SetInputsFromFile:2 "
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:SetInputsFromFile:3\n",
            CallLevel, "" );

  bool bValid = ValidateSettings();
  if( bValid ) {
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:SetInputsFromFile:3a\n",
              CallLevel, "" );

    SetSimulationDuration( 0 );  //Reset Duration

    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:SetInputsFromFile:3b\n",
              CallLevel, "" );

    SetStartDate( GetJulianDays(StartMonth) + StartDay + StartHour );

    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:SetInputsFromFile:3c\n",
              CallLevel, "" );

    bValid = Execute_InitiateTerminate();

    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:SetInputsFromFile:3d\n",
              CallLevel, "" );
  }
...
{ //Farsite::ProcessNameValue
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:ProcessNameValue:1 Processing "
            "name=%s value=%s\n", CallLevel, "", name, value );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:ProcessNameValue:2\n",
            CallLevel, "" );
  ...
  //Suppression keywords added Aug30, 2010.
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:ProcessNameValue:2a %s\n",
              CallLevel, "", value );
    ...
    long NumCoords = ProcessAttackString( value, GROUND, &ResourceNum,
                                          &AttackType, Coords );
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:ProcessNameValue:2b "
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:ProcessNameValue:2c\n",
              CallLevel, "" );
  }
...
//    SetupAirAttack( ResourceNum, CoverageLevel, Duration, Coords );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:ProcessNameValue:3\n",
            CallLevel, "" );
...
/*============================================================================
  Farsite::ValidateSettings
  Used in Farsite::SetInputsFromFile() to do error-checking on
...
/*============================================================================
  Farsite::ProcessAttackString
*/
long Farsite::ProcessAttackString( char *value, int ResourceType,
                                   long *ResourceNum,
...
{ //Farsite::ProcessAttackString
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite::ProcessAttackString:1 "
            "value=%s\n", CallLevel, "", value );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite::ProcessAttackString:2 "
  ...
  char *ps = strtok( NULL, " \t\n\r" );
  if( ! ps ) {
    printf( "## Farsite::ProcessAttackString: Missing or invalid "
...
/*============================================================================
  Farsite::Execute_InitiateTerminate
  Displays landscape window and enables/disables menu selections.
*/
bool Farsite::Execute_InitiateTerminate()
{ //Farsite::Execute_InitiateTerminate
  CallLevel++;
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:Execute_InitiateTerminate:1\n",
            CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:Execute_InitiateTerminate:3\n",
            CallLevel, "" );
  CallLevel--;
...
//============================================================================
void Farsite::FlatOpenProject()
{ //Farsite::FlatOpenProject
  CallLevel++;

  if( Verbose > CallLevel )
     printf( "%*sFarsite:Farsite:FlatOpenProject:1 "
             "Setting Duration....\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FlatOpenProject:2 "
            "Executing 'Start/Restart Simulation'....\n", CallLevel, "" );

  Execute_StartRestart();

  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FlatOpenProject:3 Exiting....\n",
            CallLevel, "" );
...
/*============================================================================
  Farsite::Execute_StartRestart
  Starts the simulation process loop.
*/
void Farsite::Execute_StartRestart()
{ //Farsite::Execute_StartRestart
  CallLevel++;
  ...
  if( RealFires > 0 ) {
    if( SimulationDuration == 0.0 ) {
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:Execute_StartRestart:1 "
      ...
      for( i = 1; i <= grid->NumAllCats[3]; i++ ) {
        if( Verbose > CallLevel )
          printf( "%*sFarsite:Farsite:Execute_StartRestart:1a "
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:Execute_StartRestart:2 "
                "\n", CallLevel, "" );
      if( HaveGroundFuels() ) {
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:Execute_StartRestart:3 "
                "\n", CallLevel, "" );
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:Execute_StartRestart:4 "
                "\n", CallLevel, "" );
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:Execute_StartRestart:5 "
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:Execute_StartRestart:6 "
...
//============================================================================
bool Farsite::OpenLandscapeFile()
{ //Farsite::OpenLandscapeFile
...
//============================================================================
void Farsite::Execute_ResumeSuspend()
{ //Farsite::Execute_ResumeSuspend
  if( SIMULATE_GO ) {
...
/*============================================================================
  Farsite::FarsiteSimulationLoop
  This function activates the FARSITE loop and checks for messages between
  iterations.
*/
void Farsite::FarsiteSimulationLoop()
{ //Farsite::FarsiteSimulationLoop
  CallLevel++;

  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:1\n",
            CallLevel, "" );

  CheckSteps();    //Check visual and actual timestep for changes
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2 "
            "maximum=%ld\n", CallLevel, "", maximum );

  while( burn.SIMTIME <= maximum ) {
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2a "
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2b "
              "\n", CallLevel, "" );

    if( FARSITE_GO ) {
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2b1 "
                "\n", CallLevel, "" );
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2b3 "
                "\n", CallLevel, "" );

      if( ProcNum == 3 ) {
        if( Verbose > CallLevel )
          printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2b3a "
                  "\n", CallLevel, "" );
        ...
        if( Verbose > CallLevel )
          printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2b3b "
                  "\n", CallLevel, "" );
        ...
        if( Verbose > CallLevel )
          printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2b3c "
                  "\n", CallLevel, "" );
      }
    }
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2c "
    ...
    if( burn.SIMTIME > maximum ) {
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2c1 "
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:2d "
              "Sim time = %f maximum=%ld\n",
              CallLevel, "", burn.SIMTIME, maximum );
  }  //End while loop

  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:3 "
            "Exited main loop\n", CallLevel, "" );
  ...
  if( burn.SIMTIME > maximum ) {
    ...
    if( GetShapeMake() ) WriteGISLogFile( 2 );
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteSimulationLoop:3a "
              "SIMULATION COMPLETED\n", CallLevel, "" );
  }
...
//============================================================================
void Farsite::WriteGISLogFile( long LogType )
{ //Farsite::WriteGISLogFile
  char LogFile[256];
  ...
  fprintf(logfile, "%s %s\n", "Simulation Ended (Day Hour:Min):", CurTime);
  fprintf(logfile, "%s %s\n\n", "Elapsed Time (Days Hours:Mins):", ElTime);
  fprintf(logfile, "%s %lf\n", "Actual Time Step (min):",
          GetActualTimeStep());
  fprintf(logfile, "%s %lf\n", "Visible Time Step (min):",
          GetVisibleTimeStep());
  fprintf(logfile, "%s %lf\n", "Perimeter Resolution (m):", GetPerimRes());
...
/*============================================================================
  Farsite::SaveIgnitions
  For saving ignitions before restarting.
*/
void Farsite::SaveIgnitions()
{ //Farsite::SaveIgnitions
  CallLevel++;
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:SaveIgnitions:1\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:SaveIgnitions:2\n",
            CallLevel, "" );
  CallLevel--;
} //Farsite::SaveIgnitions

/*============================================================================
  Farsite::FarsiteProcess1
  This function contains the Farsite process control.
*/
bool Farsite::FarsiteProcess1()
{ //Farsite::FarsiteProcess1
  long InOut, FireNum;
  CallLevel++;
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess1:1 "
  ...
  if( DistanceCheckMethod(GETVAL) == DISTCHECK_SIMLEVEL ) {
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcess1:1a\n",
              CallLevel, "" );
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcess1:1b\n", CallLevel, "" );
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcess1:1c\n", CallLevel, "" );

    TotalAreaBurned = 0; //Init
    for( CurrentFire = NextFireAfterInterrupt; CurrentFire < GetNumFires();
         CurrentFire++ ) {    //For all fires
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteProcess1:1c1\n", CallLevel, "" );

      if( (InOut = GetInout(CurrentFire)) == 0 ) continue;

      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteProcess1:1c2\n", CallLevel, "" );

      if( OutputVectsAsGrown )
        WriteVectorOutputs(); //Output perimeters to screen and/or vector file

      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteProcess1:1c3 "
      ...
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:FarsiteProcess1:1c4\n", CallLevel, "" );
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcess1:1d\n", CallLevel, "" );
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcess1:1e\n", CallLevel, "" );

    NextFireAfterInterrupt = 0;
    AddDownTime( -1.0 );
  }
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess1:2\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess1:3\n", CallLevel, "" );
...
/*============================================================================
  Farsite::FarsiteProcessSpots
  This function contains the Farsite process control.
*/
bool Farsite::FarsiteProcessSpots()
{ //Farsite::FarsiteProcessSpots
  CallLevel++;
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcessSpots:1 "
            "burn.SIMTIME=%lf\n",
            CallLevel, "", burn.SIMTIME );

  if( ! IN_BURNPERIOD ) {
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcessSpots:1a "
              "burn.SIMTIME=%lf\n",
              CallLevel, "", burn.SIMTIME );
    CallLevel--;
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcessSpots:2 "
            "burn.SIMTIME=%lf\n",
            CallLevel, "", burn.SIMTIME );
  CallLevel--;
...
long Farsite::InsertSpotFire(double xpt, double ypt)
{
  long k, newspots;
  long NumVertex;
...
//============================================================================
bool Farsite::FarsiteProcess2()
{ //Farsite::FarsiteProcess2
  if( ! IN_BURNPERIOD ) {
    ProcNum = 3;
...
/*============================================================================
  Farsite::FarsiteProcess3
  This function processes fire mergers.
*/
void Farsite::FarsiteProcess3()
{ //Farsite::FarsiteProcess3
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess3:1 "
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess3:2\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess3:3\n", CallLevel, "" );

  //If object for handling group air attacks exists
  if( gaat ) {
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:FarsiteProcess3:3a "
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess3:4\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess3:6\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:FarsiteProcess3:7 "
            "burn.SIMTIME=%lf\n",
            CallLevel, "", burn.SIMTIME );
...
void Farsite::CheckStopLocations()
{
  bool found = false;
  long i, j;
  double xpt, ypt;
...
//============================================================================
void Farsite::WriteOutputs( int type )
{ //Farsite::WriteOutputs
  CallLevel++;

  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:WriteOutputs:1\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:WriteOutputs:2\n", CallLevel, "" );

  CallLevel--;
} //Farsite::WriteOutputs


void Farsite::StartMoistThread()
{
...
unsigned Farsite::RunMoistThread(void* FarIntFace)
{
  static_cast <Farsite*>(FarIntFace)->MoistThread();

  return 1;
}


void Farsite::MoistThread()
{
	double Interval;
...
bool Farsite::PreCalculateFuelMoistures()
{
	char Line[64] = "";
	long mo, dy, hr, mn;
	double SimTime;
...
//============================================================================
void Farsite::CheckSteps()
{ //Farsite::CheckSteps
  CallLevel++;

  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:CheckSteps:1\n", CallLevel, "" );
...
maximum = 30; //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA To stop sim after first perim!!! AAA
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:CheckSteps:1a maximum=%ld\n",
              CallLevel, "", maximum );
    ...
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:CheckSteps:1b\n", CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:CheckSteps:2\n", CallLevel, "" );

  CallLevel--;
} //Farsite::CheckSteps

//============================================================================
void Farsite::WriteVectorOutputs()
{ 
...
//============================================================================
bool Farsite::PreCalculateFuelMoisturesNoUI()
{ //Farsite::PreCalculateFuelMoisturesNoUI
  long mo, dy, hr, mn;
  double SimTime;
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:1\n",
            CallLevel, "" );
  ...
  //taken care of.
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:2\n",
            CallLevel, "" );

  //If weather etc. changed.
  if( EnvironmentChanged(GETVAL, 0, SIZECLASS_10HR) ) LastFMCalcTime = 0;
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:3 "
            "\n", CallLevel, "" );
  if( burn.env->CalcMapFuelMoistures(SimTime) ) {}
  else {
    if( Verbose > CallLevel )
      printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:3a "
              "\n", CallLevel, "" );
    if( LastFMCalcTime >= maximum ) {
      if( Verbose > CallLevel )
        printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:"
                "3a1\n", CallLevel, "" );
      CallLevel--;
      return false;
    }
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:4\n",
            CallLevel, "" );
  ...
  if( Verbose > CallLevel )
    printf( "%*sFarsite:Farsite:PreCalculateFuelMoisturesNoUI:5\n",
            CallLevel, "" );
  CallLevel--;
  return true;
} //Farsite::PreCalculateFuelMoisturesNoUI
...
//============================================================================
void Farsite::SaveSettingsToFile( const char *fname )
{ //Farsite::SaveSettingsToFile
  FILE* outfile;
  char Text[8]="";
  char String[256]="";
  char AclFile[256]="";
